...
from models.pmcgnn import PMCGNN
...
def train_pyg(
    ...
    if config.write_checkpoint:
        ...
        final_checkpoint_handler = Checkpoint(
            to_save,
            DiskSaver(last_checkpoint_dir, create_dir=True, require_empty=False),
            n_saved=1,
            global_step_transform=lambda *_: trainer.state.max_epochs,
        )
        evaluator.add_event_handler(Events.EPOCH_COMPLETED, handler)
        trainer.add_event_handler(Events.COMPLETED, final_checkpoint_handler)
    ...
    with torch.no_grad():
        ...
        for dat, id in tqdm(zip(test_loader, ids)):
            ...
            out_data = out_data * std + mean  # std=1 mean=0
            if config.write_predictions:
                f.write("%s, %6f, %6f\n" % (id, target, out_data))
