...
class RRWPLinearNodeEncoder(torch.nn.Module):
    ...
    def __init__(self, emb_dim, out_dim, use_bias=False, batchnorm=False, layernorm=False, pe_name="rrwp"):
        ...
        self.encoderdrop = nn.Dropout(0.5)
        
    def forward(self, batch, node_enc):
        # Encode just the first dimension if more exist
        rrwp = batch
        rrwp = self.fc(rrwp)

        rrwp = self.encoderdrop(rrwp)
...
class RRWPLinearEdgeEncoder(torch.nn.Module):
    ...
    def forward(self, rrwp_idx, rrwp_val, edge_index, edge_attr,node_nums):
        ...
        if self.overwrite_old_attr:
            ...
            # edge_index, edge_attr = add_remaining_self_loops(edge_index, edge_attr, num_nodes=node_nums,
            #                                                  fill_value=0.)
        ...
        rrwp_edge_index, rrwp_edge_attr = out_idx, out_val
        return rrwp_edge_index, rrwp_edge_attr
